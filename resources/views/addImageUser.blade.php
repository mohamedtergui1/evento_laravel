<x-guest-layout>
    <div class=" h-48 mx-auto w-1/2 pb-5 dark:bg-slate-800 gap-6 flex items-center justify-center">
        <div
            class="bg-gray-100 w-full dark:bg-gray-700 relative shadow-xl overflow-hidden hover:shadow-2xl group rounded-xl p-5 transition-all duration-500 transform">
            <div   class="flex items-center gap-4">
                <img id="imagePreview" src="{{asset("uploads/users/".auth()->user()->image)}}"
                ...
                <div  class="flex justify-end" >
                    <button class="bg-blue-500 rounded-xl px-2 py-1"   onclick="submitUpdateImageUser()" >
                        submit
                    </button>
        </div>
            </div>
            ...
            <div
                class="absolute group-hover:bottom-1 delay-300 -bottom-16 transition-all duration-500 bg-gray-600 dark:bg-gray-100 right-1 rounded-lg">
    ...
    <div class="w-[400px] relative border-2 border-gray-300 border-dashed rounded-lg p-6" id="dropzone">
        <form id="updateImageForm" method="POST" action="" enctype="multipart/form-data" >
            @csrf
            @method("PUT")
        <input onchange="previewImageUser(event)" name="image" type="file" class="absolute inset-0 w-full h-full opacity-0 z-50" />
    </form>
