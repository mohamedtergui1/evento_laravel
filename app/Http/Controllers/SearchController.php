...
class SearchController extends Controller
{
    ...
    else $events = Event::with("user","category")->where("title","like",'%'.$request->search_string.'%')->where("status","accepted")->latest() ->paginate(6);;
    ...
    //     $events = Event::with("user","category")->paginate(9);
    //     if($events->count()) return response()->json([
    //         "status" => true
    //         ,
    //         "events" => $events
